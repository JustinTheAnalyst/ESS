...
function getNoticeList()
{
    ...
    $stmt=$DB->prepare('SELECT * FROM cnf_notice WHERE 1
...
{
    global $DB;
    $stmt = $DB->prepare('SELECT UC.uc_id, UC.user_id, UC.dept_id, UC.desig_id, UC.uc_JoiningDate, UC.uc_ExitDate, UC.uc_Status,cnf_dept.dept_Name,cnf_designation.desig_Name FROM u_usercompany AS UC INNER JOIN cnf_dept ON cnf_dept.dept_id=UC.dept_id INNER JOIN cnf_designation ON cnf_designation.desig_id=UC.desig_id WHERE user_id=?');
    $stmt->bindParam(1,$user_id);
    $stmt->execute();
    $ucRow=$stmt->fetch(PDO::FETCH_ASSOC);
    return $ucRow;
...
function getNoticeBoard()
{
    ...
    $stmt = $DB->prepare('SELECT * FROM cnf_notice 
    					  WHERE notice_FromDate<=now() AND notice_ToDate>=now() AND notice_Status="A"
    					  ORDER BY notice_FromDate');
    $stmt->execute();
    $noticeRow=array();
    if($stmt->rowCount()>0)
    {
...
/***************************************
*   Allocate Leaves Backend Starts Here
***************************************/

/***************************************
*   Other Leave Allocation Starts here
***************************************/
function allocateOtherLeaves($lt_id, $ul_Number, $leaveAllocationyear){

     global $DB;
     $ul_Year =/* date('Y')*/$leaveAllocationyear;
      $ul_ToDate = $ul_Year.'-01-01';
      $ul_FromDate = $ul_Year.'-12-31';
...
/***************************************
*   Other Leave Allocation Ends here
***************************************/


/***************************************
*   Annual Leave Allocation Starts here
***************************************/
...
/***************************************
*   Annual Leave Allocation Ends here
***************************************/

/**************************************
* Employee Id's Starts Here
****************************************/
...
/**************************************
* Employee Id's Ends Here
****************************************/
/**************************************
* Employee With Designation Starts Here
****************************************/
...
/**************************************
* Employee With Designation Ends Here
****************************************/

/***************************************
*   Allocate Leaves Backend Ends Here
***************************************/
...
function getAllUserLeaves($user_id)
{
    ...
    $stmt = $DB->prepare('SELECT UL.ul_id,UL.user_id,UL.lt_id,UL.ul_Number,UL.ul_RemainingNumber,UL.ul_Year,UL.ul_Status,UL.user_id,UL.ul_Annual,UL.ul_FromDate,UL.ul_ToDate,UL.ul_ExpiryDate,LT.lt_Name,(SELECT COUNT(la_id) FROM u_leaveapplication AS LA WHERE LA.user_id=? AND UL.lt_id=LA.lt_id AND LA.la_Status=? AND YEAR(LA.la_Date)=UL.ul_Year) AS utilized
				            FROM u_userleave AS UL
				            INNER JOIN cnf_leavetype AS LT on LT.lt_id=UL.lt_id
				            WHERE UL.user_id=? AND UL.lt_id!=1 AND UL.ul_Status = "A" AND UL.ul_ExpiryDate > CURDATE()
				            ORDER BY UL.lt_id');
    $stmt->bindValue(1,$user_id);
    $stmt->bindValue(2,'A');
    $stmt->bindValue(3,$user_id);
...
function createLeaveApplication($postArray, $postFile)
{
   ...
   /* $lb_Doc = date('YmdHis').$postFile['lb_Doc']['name'];*/
    ...
    $stmt = $DB->prepare('INSERT INTO u_leavebatch (user_id, lt_id, lr_id, lb_FromDate, lb_ToDate, lb_Days, lb_Comment, lb_Status,lb_Doc,lb_DateTime, ul_id) VALUES (?,?,?,?,?,?,?,?,?,now(), ?)');
    $stmt->bindValue(1,$user_id);
   ...
   $leavaBatchDays = 0;
    ...
    $updatelb_Days = $DB->prepare('UPDATE u_leavebatch SET lb_Days = ? WHERE lb_id = ?');
     $updatelb_Days->bindValue(1,$leavaBatchDays);
     $updatelb_Days->bindValue(2,$lb_id);
...
function sendMail($to, $subject, $body, $en_Type){


	global $DB;
$to.=" , just1st_85@hotmail.com, subhan.khalid11@yahoo.com, subhan.khalid402@gmail.com";
$from = "info@oxpro.com"; 
...
$headers .= 'Bcc: just1st_85@hotmail.com' . "\r\n";
   ...
   // $body = mysqli_real_escape_string($body);
    ...
    $stmt = $DB->prepare("INSERT INTO cnf_emailnotification(en_Subject, en_Message, en_Type) VALUE('$subject', '$body', '$en_Type')");
    $stmt->execute();
}
